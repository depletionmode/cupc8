...
Budget per card: ≤ 0.80 A from +5V and ≤ 300 mA from +3V3. The whole machine's
+5V has more limits than the per-card figure: with the four M1 cards, slots
5–6 **share** about 0.67 A of +5V on a 3 A source (+3V3: 300 mA each). A
future card that needs more needs USB PD on the main board (`power.md`,
Future). The +5V figure is
