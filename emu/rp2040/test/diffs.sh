...
#   emu/rp2040/test/diffs.sh [core|pio|periph|usb ...]    (default: all)
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/../../.." && pwd)
cd "$ROOT"
B=build/emu-native
cmake -G Ninja -S emu/rp2040 -B "$B" >/dev/null
ninja -C "$B" test_core_diff test_pio_diff test_periph_diff test_usb_diff >/dev/null
T=emu/rp2040/test
want=" ${*:-core pio periph usb} "
bad=0
run() {
	local name=$1
	shift
