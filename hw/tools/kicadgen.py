...
def build_board(comps, nets, placement, outline, layers=2, zones=("GND",), graphics=(), edge=None,
    ...
    explicit = any(not isinstance(z, str) for z in zones)
    if layers == 4 and not explicit:
        ...
        # (A board that names its zones' layers, as the CPU card, gets only
        # those: plain nets then pour the outer layers.)
    ...
    for zone in zones:
        ...
        zone_layers = [board.GetLayerID(n) for n in names] if names else copper
        for layer in zone_layers:
            if names and layer not in (pcbnew.F_Cu, pcbnew.B_Cu):
...
def pipeline(name, schematic, placement, outline, out=None, zones=("/GND",), power_nets=(),
             ...
             io_card=False, fine_nets=(), preroute=None, route_tries=3, prepare=None, presence=None):
    ...
    step("board", build)
    step("autoroute", lambda: autoroute(state["b"], out, passes, pours=pour_nets, tries=route_tries))
