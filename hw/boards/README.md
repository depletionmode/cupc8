...
- **Inner layers on 4-layer cards (both opt-in):**
  ...
  - Neither (plain zones, the RP2040 slot cards): In1.Cu a solid plane of the
    pour net, In2.Cu routed and poured. `preroute=` (as `prepare=`) and
    `route_tries=` are these cards' names for the pre-routing and the
    Freerouting retries.
