...
def main():
    ...
    ap.add_argument("input", help="prog.s (assembled) or prog.bin (for $7000)")
    ap.add_argument("-o", "--output", required=True)
    a = ap.parse_args()
    body = build(a.input) if a.input.endswith(".s") else open(a.input, "rb").read()
    if body.startswith(HEADER[:3]):
        sys.exit("mkprg.py: %s already has a header" % a.input)
